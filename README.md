...
## 週次レポート（GitHub Actions の Job Summary）

毎回の実行で、Actions実行画面の Summary に週次レポートを出力する（追加DB不要）:
- **💰 収益（アフィリンククリック）**: 記事別のAmazon/楽天リンククリック数と、セッションあたりの率。率が低く流入が多い記事＝導線改善の余地、率が高く流入が少ない記事＝集客強化の余地。
- **📈 SEOダイジェスト**: 直近28日 vs 前28日で、順位が上がった/下がった記事、クリック増減、新しく流入し始めたクエリ。
- **🛠 リライト効果測定**: 過去のAI提案（対象URL・提案日・提案時メトリクスは下書き本文コメント由来）と現在のGSC値を比較（順位/CTR/Imp の before→現在）。状態（未レビュー/却下）も表示。

ローカル実行時は標準出力に出る。`src/report/summary.ts`。

### 提案履歴の永続化（data/proposals.json）

WordPressのゴミ箱は約30日で自動完全削除されるため、そこに置いた提案記録も消える。
効果測定の台帳を長期に保つため、提案履歴（提案日・対象URL・提案時の順位/CTR/Imp）を
**`data/proposals.json` に追記して保持**する（GitHub Actionsが実行ごとにコミット）。

- **WPのゴミ箱** = 再提案の抑止 + 約30日のクールダウン（運用はそのままでOK）
- **履歴JSON** = 恒久的な台帳。30日を過ぎても効果測定の行が消えない
- 運用上「リライトを反映したらゴミ箱へ入れる」を続けて問題ない。レポートの状態は
  「未レビュー（下書き残存）」/「対応済み（ゴミ箱・削除済み含む）」で表示される
...
| `REVENUE_WEIGHT` | 設定 | 収益ポテンシャルをスコアに反映する強さ（既定 `1`＝最大2倍。`0`で無効） |
| `DRY_RUN` | 設定 | `true`=投稿せずログのみ / `false`=下書き投稿する |
| `USE_BATCH` | 設定 | `true`=Batch API（50%オフ・既定） / `false`=同期API（デバッグ用） |
| | | ※ Batchが失敗・タイムアウトした場合は**自動で同期生成にフォールバック**（週次実行を落とさない） |
...
| Batchがタイムアウト | Anthropic側のキュー遅延。タイムアウト時はBatchを自動キャンセルし、**同期生成にフォールバック**して下書きは作られる（50%オフを逃す分だけコスト増）。待ち時間は `BATCH_MAX_WAIT_MS`（既定30分）で調整。GHA実行時間も消費するので延長しすぎない |
